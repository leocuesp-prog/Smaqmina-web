...
function cargarTabla() {
    ...
    fetch("http://localhost:3000/herramientas/sujecion")
        .then(res => res.json())
        .then(herramientas => {
            ...
            herramientas.forEach((herramienta, i) => {
                ...
                nombreCell.innerText = herramienta.nombre_herramienta_sujecion;

                let cantidadCell = fila.insertCell(2);
                cantidadCell.innerText = herramienta.cantidad_buena_sujecion;
                ...
                select.addEventListener("change", function () {
                    ...
                    if (select.value === "Regular") {
                        cantidadCell.innerText = herramienta.cantidad_regular_sujecion;
                    }
                    if (select.value === "Malo") {
                        cantidadCell.innerText = herramienta.cantidad_mala_sujecion;
                    }
                });
                ...
                if (usuarioActivo) {
                    ...
                    botonEliminar.addEventListener("click", function () {
                        if (confirm("¿Estás seguro de que deseas eliminar esta herramienta?")) {
                            fetch(`http://localhost:3000/herramientas/sujecion/${herramienta.id_herramienta_sujecion}`, {
                                method: "DELETE"
                            })
                                .then(res => res.json())
                                .then(datos => {
